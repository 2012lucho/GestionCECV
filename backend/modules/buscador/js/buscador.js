...
function CreaVista(data,Elemento,n,CamposM,V,Cindice){
	...
	for (item=0;item<data.length;item++){
		...
		if (V['Control']!=''){
			vista+='</div><div class="col-xs-1"><input type='+V['Control']+' class="BI" ></div>';
		}		
		
		$('#'+Elemento+n).append('<div class="RB col-xs-12" id="'+Elemento+n+'-'+data[item][Cindice]+'"><div class="col-xs-11">'+vista+'</div>');
	}
}
...
function InicializarBuscador(Config,CamposM,Vista){
	...
	//si se presionan algun checkbox
	/*$('.BI').click (function(){
    	if ( $(this).is(':checked') ) {
