...
function CreaVista(data,Elemento,n,CamposM,V,Cindice){
	...
	for (item=0;item<data.length;item++){
		...
		if (V['Control']!=''){
			//creamos el control			
			vista+='</div><div class="col-xs-1"><input id="C'+Elemento+n+item+'" type='+V['Control']+' class="BI" ></div>';	
		}		
		// creamos registro 
		$('#'+Elemento+n).append('<div class="RB col-xs-12" id="'+Elemento+n+'-'+data[item][Cindice]+'"><div class="col-xs-11" id="'+Elemento+n+item+'">'+vista+'</div>');
		//asociamos evento y comportamiento
		CompartamientoControl('C'+Elemento+n+item);	
		$('#C'+Elemento+n+item).data('data-padre',Elemento+n+item);
		$('#C'+Elemento+n+item).data('idRegistro',data[item][Cindice]);
		$('#C'+Elemento+n+item).data('Control',Elemento);
	}
}
...
function CompartamientoControl(id){
	//agregamos eventos para los checkbox
	$('#'+id).change(function() {
